...
program EOB
  ...
  COMMON /group4/ Lz
  real * 8 :: M,mu,nu,a,chi_Kerr,K0,K,omega_1,omega_2
  real * 8 :: Delta_0,Delta_1,Delta_2,Delta_3,Delta_4
  real * 8 :: S_Kerr(3),norm_S_Kerr,sigma_Star(3),d_SO,d_SS
  real * 8 :: Lz

  real * 8 :: m_1,m_2,S_1(3),S_2(3),psi,zeta,Rotation(3,3),p,e,R_min,R_max,u,theta_min,theta_max
  real * 8 :: pericentre(3),apocentre(3),coordinates(3),Momentum(3)
  real * 8 :: parameters1(9),parameters2(9),g_1(8),g_2(8)
  real * 8 :: a1,a2,b1,b2,c1,c2,Z1,Z2,Energy,Carter
  real * 8 :: Difference=1.D0,epsilon = 1E-20
  real * 8 :: h,t,t_f,Running_time,Time_Begin,Time_End
  ...
  pericentre(:) = (/R_min,theta_min,0.D0/)
...
function D_Parameter(parameters,coordinates)
  ...
  real * 8 :: R,theta,omega_tilde,Delta_t,varpi_square
  real * 8 :: Delta_r,Sigma,Lambda_t,gtt,gtphi,dDeltau_du,dDeltat_dr,dDeltar_dr,dLambdat_dr
  real * 8 :: D_Metric(10),D_Parameter(10)
...
function Metric(parameters,theta)
      Implicit None
      real * 8 ,INTENT(IN):: parameters(9),theta
      real * 8 :: g_tt,g_rr,g_thetatheta,g_phiphi,g_tphi,alpha,beta,gamma
      real * 8 :: Metric(8)
...
function P_R_Dot(g,parameters,coordinates,Momentum,Derivative_of_parameters)
    implicit none
    ...
    real * 8 :: M,mu,nu,a
    real * 8 :: S_Kerr(3),norm_S_Kerr,sigma_Star(3),d_SO,d_SS
    real * 8, INTENT(IN) :: g(8),parameters(9),coordinates(3),Momentum(3),Derivative_of_parameters(10)
    real * 8 :: Q_4,The_guy_with_alpha,Energy,dH_dHeff
    real * 8 :: dalpha_dr,dbeta_dr,dgammarr_dr,dgammatt_dr,dgammapp_dr
    real * 8 :: dQ_dr,dH_Spin_dr,dSti_dr
    real * 8 :: P_R_Dot
    ...
    dH_Spin_dr = dH_dr(coordinates,Momentum)
    dSti_dr = dSpin_to_itself_dr(coordinates,Momentum)
    P_R_Dot = -dH_dHeff*(dbeta_dr*Momentum(3)+dalpha_dr*The_guy_with_alpha+g(6)*(dgammarr_dr*Momentum(1)**2+&
              ...
              dH_Spin_dr/mu - 4*d_SS*nu*dot_product(S_Kerr,sigma_Star)/coordinates(1)**5  + dSti_dr)
    return
    end function
function p_Theta_Dot(g,parameters,coordinates,Momentum,Derivative_of_parameters)
    implicit none
    COMMON /group1/ M,mu,nu,a
    COMMON /group3/ S_Kerr,norm_S_Kerr,sigma_Star,d_SO,d_SS
    real * 8 :: M,mu,nu,a
    ...
    real * 8, INTENT(IN) :: g(8),parameters(9),coordinates(3),Momentum(3),Derivative_of_parameters(10)
    real * 8 :: Q_4,The_guy_with_alpha,Energy,dH_dHeff
    real * 8 :: dalpha_dth,dbeta_dth,dgammarr_dth,dgammatt_dth,dgammapp_dth
    real * 8 :: dQ_dth,dH_Spin_dth,dSti_dth
    real * 8 :: P_Theta_Dot
    ...
    dH_Spin_dth = dH_dth(coordinates,Momentum)
    dSti_dth = dSpin_to_itself_dth(coordinates,Momentum)
    P_Theta_Dot = -dH_dHeff*(dbeta_dth*Momentum(3)+dalpha_dth*The_guy_with_alpha+g(6)*(dgammarr_dth*Momentum(1)**2+&
                  ...
                  dH_Spin_dth/mu + dSti_dth)
    return
    end function
function R_Dot(g,parameters,coordinates,Momentum)
    implicit none
    COMMON /group1/ M,mu,nu
    COMMON /group3/ S_Kerr,norm_S_Kerr,sigma_Star,d_SO,d_SS
    real * 8 :: M,mu,nu
    real * 8 :: S_Kerr(3),norm_S_Kerr,sigma_Star(3),d_SO,d_SS
    real * 8, INTENT(IN) :: g(8),parameters(9),coordinates(3),Momentum(3)
    real * 8 :: Q_4,The_guy_with_alpha,Energy,dH_dHeff,dQ_dpr,dH_Spin_dpr,dSti_dpr
    real * 8 :: R_Dot
    ...
    dH_Spin_dpr = dH_dpr(coordinates,Momentum)
    dSti_dpr = dSpin_to_itself_dpr(coordinates,Momentum)
    R_Dot =  dH_dHeff*(g(6)*(2*g(2)*Momentum(1)+dQ_dpr)/(2*The_guy_with_alpha)+dH_Spin_dpr/mu+dSti_dpr)
    return
    end function
function Theta_Dot(g,parameters,coordinates,Momentum)
    implicit none
    COMMON /group1/ M,mu,nu
    COMMON /group3/ S_Kerr,norm_S_Kerr,sigma_Star,d_SO,d_SS
    real * 8 :: M,mu,nu
    real * 8 :: S_Kerr(3),norm_S_Kerr,sigma_Star(3),d_SO,d_SS
    real * 8, INTENT(IN) :: g(8),parameters(9),coordinates(3),Momentum(3)
    real * 8 :: Q_4,The_guy_with_alpha,Energy,dH_dHeff,dH_Spin_dpth,dSti_dpth
    real * 8 :: Theta_Dot
    ...
    Energy = g(7)*Momentum(3) + g(6)*The_guy_with_alpha + &
             ...
             Spin_to_itself(parameters,coordinates,Momentum)
    dH_dHeff = 1/Sqrt(1+2*nu*(Energy-1))
    dH_Spin_dpth = dH_dpth(coordinates,Momentum)
    dSti_dpth = dSpin_to_itself_dpth(coordinates,Momentum)
    Theta_Dot = dH_dHeff*(g(6)*g(3)*Momentum(2)/The_guy_with_alpha+dH_Spin_dpth/mu+dSti_dpth)
    return
    end function
function Phi_Dot(g,parameters,coordinates,Momentum)
    implicit none
    COMMON /group1/ M,mu,nu
    COMMON /group3/ S_Kerr,norm_S_Kerr,sigma_Star,d_SO,d_SS
    real * 8 :: M,mu,nu
    real * 8 :: S_Kerr(3),norm_S_Kerr,sigma_Star(3),d_SO,d_SS
    real * 8, INTENT(IN) :: g(8),parameters(9),coordinates(3),Momentum(3)
    real * 8 :: Q_4,The_guy_with_alpha,Energy,dH_dHeff,dH_Spin_dpph,dSti_dpph
    real * 8 :: Phi_Dot
    ...
    Energy = g(7)*Momentum(3) + g(6)*The_guy_with_alpha + &
             ...
             Spin_to_itself(parameters,coordinates,Momentum)
    dH_dHeff = 1/Sqrt(1+2*nu*(Energy-1))
    dH_Spin_dpph = dH_dpph(coordinates,Momentum) 
    dSti_dpph = dSpin_to_itself_dpph(coordinates,Momentum)
    Phi_Dot = dH_dHeff*(g(7)+g(6)*g(8)*Momentum(3)/The_guy_with_alpha+dH_Spin_dpph/mu+dSti_dpph)
    return
    end function
!——————————————————————————————————————S_Star————————————————————————————————————————————————————————————
...
function S_Star(parameters,coordinates,Momentum)
    ...
    real * 8 :: g(8),R,theta,u,p_R,p_theta,p_phi,Q
...
function H_SO_SS(parameters,coordinates,Momentum)
    ...
    real * 8 ,INTENT(IN) :: parameters(9),coordinates(3),Momentum(3)
    real * 8 :: R,theta,phi,u,n(3),p_R,p_theta,p_phi
    real * 8 :: Sigma,varpi_square,omega_tilde,Delta_t,Delta_r,Lambda_t,dDeltat_dr,dLambdat_dr
    real * 8 :: omega,omega_r,omega_cos,e_2nu,e_2mutilde,B_tilde,B_rtilde,J_tilde,mu_r,mu_cos,nu_r,nu_cos
    real * 8 :: Q,S(3),xi(3),v(3)
    real * 8 :: H_SO,H_SS,H_SO_SS 
...
function Spin_to_itself(parameters,coordinates,Momentum)
    ...
    real * 8, INTENT(IN) :: parameters(9),coordinates(3),Momentum(3)
    real * 8 :: R,theta,phi
    real * 8 :: X(3),S_Star_(3),norm_S_Star
...
function dH_dr(coordinates,Momentum)
    implicit none
    COMMON /group1/ M
    real * 8 :: M
    real * 8 , INTENT(IN)  ::  coordinates(3),Momentum(3)
    real * 8 :: f1,f2,h,Temp_X(3),Temp_P(3),parameters(9)
    real * 8 :: dH_dr
    h = 1E-4 * M
    Temp_X(:) = coordinates(:)
...
function dH_dth(coordinates,Momentum)   
    implicit none
    COMMON /group1/ M
    real * 8 :: M
    real * 8 , INTENT(IN)  ::  coordinates(3),Momentum(3)
...
function dH_dpr(coordinates,Momentum)
    implicit none
    COMMON /group1/ M
    real * 8 :: M
    real * 8 , INTENT(IN)  ::  coordinates(3),Momentum(3)
...
function dH_dpth(coordinates,Momentum)
    implicit none
    COMMON /group1/ M
    real * 8 :: M
    real * 8 , INTENT(IN)  ::  coordinates(3),Momentum(3)
...
function dH_dpph(coordinates,Momentum)
    implicit none
    COMMON /group1/ M
    real * 8 :: M
    real * 8 , INTENT(IN)  ::  coordinates(3),Momentum(3)
...
function dSpin_to_itself_dr(coordinates,Momentum)
    implicit none
    COMMON /group1/ M
    real * 8 :: M
    real * 8 , INTENT(IN)  ::  coordinates(3),Momentum(3)
    real * 8 :: f1,f2,h,Temp_X(3),Temp_P(3),parameters(9)
    real * 8 :: dSpin_to_itself_dr
    h = 1E-4 * M
    Temp_X(:) = coordinates(:)
    ...
    dSpin_to_itself_dr = (f1-f2)/h
    return
    end function
function dSpin_to_itself_dth(coordinates,Momentum)
    implicit none
    COMMON /group1/ M
    real * 8 :: M
    real * 8 , INTENT(IN)  ::  coordinates(3),Momentum(3)
    real * 8 :: f1,f2,h,Temp_X(3),Temp_P(3),parameters(9)
    real * 8 :: dSpin_to_itself_dth
    h = 1E-4 * M
    Temp_X(:) = coordinates(:)
    ...
    parameters(:) = Parameter_Calculator(Temp_X)
    f2 = Spin_to_itself(parameters,Temp_X,Momentum)
    dSpin_to_itself_dth = (f1-f2)/h
    return
    end function
function dSpin_to_itself_dpr(coordinates,Momentum)
    implicit none
    COMMON /group1/ M
    real * 8 :: M
    real * 8 , INTENT(IN)  ::  coordinates(3),Momentum(3)
    real * 8 :: f1,f2,h,Temp_X(3),Temp_P(3),parameters(9)
    real * 8 :: dSpin_to_itself_dpr
    h = 1E-4 * M
    Temp_P(:) = Momentum(:)
    parameters(:) = Parameter_Calculator(coordinates)
    ...
    dSpin_to_itself_dpr = (f1-f2)/h
    return
    end function
function dSpin_to_itself_dpth(coordinates,Momentum)
    implicit none
    COMMON /group1/ M
    real * 8 :: M
    real * 8 , INTENT(IN)  ::  coordinates(3),Momentum(3)
    real * 8 :: f1,f2,h,Temp_X(3),Temp_P(3),parameters(9)
    real * 8 :: dSpin_to_itself_dpth
    h = 1E-4 * M
    Temp_P(:) = Momentum(:)
    parameters(:) = Parameter_Calculator(coordinates)
    ...
    dSpin_to_itself_dpth = (f1-f2)/h
    return
    end function
function dSpin_to_itself_dpph(coordinates,Momentum)
    implicit none
    COMMON /group1/ M
    real * 8 :: M
    real * 8 , INTENT(IN)  ::  coordinates(3),Momentum(3)
    real * 8 :: f1,f2,h,Temp_X(3),Temp_P(3),parameters(9)
    real * 8 :: dSpin_to_itself_dpph
...
Subroutine Evolution_RK45(h,coordinates,Momentum,t,t_f)
  ...
  real * 8 :: X(5),X1(5),g(8)
  real * 8 :: Y0(5),Y1(5),Y2(5),Y3(5),Y4(5),Y5(5)
  ...
  do while (t .le. t_f)
    ...
    do i = 1,5
      X1(i) = X(i) + h*1D0*Y0(i)/4D0
    end do 
    call Differential_Eq(X1,Y1)

    do i = 1,5
      X1(i) = X(i) + h*(3D0*Y0(i)+9D0*Y1(i))/32D0
    end do 
    call Differential_Eq(X1,Y2)

    do i = 1,5
      X1(i) = X(i) + h*(1932D0*Y0(i)-7200D0*Y1(i)+7296D0*Y2(i))/2197D0
    end do 
    call Differential_Eq(X1,Y3)

    do i = 1,5
      X1(i) = X(i) + h*(439D0/216D0*Y0(i)-8D0*Y1(i)+3680D0/513D0*Y2(i)-845D0/4104D0*Y3(i))
    end do
    call Differential_Eq(X1,Y4)

    do i = 1,5
      X1(i) = X(i) + h*(-8D0/27D0*Y0(i)+2D0*Y1(i)-3544D0/2565D0*Y2(i)+1859D0/4104D0*Y3(i)-11D0/40D0*Y4(i))
    end do
    call Differential_Eq(X1,Y5)

    do i = 1,5
      X(i) = X(i) + h*(16D0/135D0*Y0(i)+6656D0/12825D0*Y2(i) + 28561D0/56430D0*Y3(i) -&
                      9D0/50D0*Y4(i) + 2D0/55D0*Y5(i))
    end do   
...
Subroutine Evolution_RK78(h,coordinates,Momentum,t,t_f)
  ...
  real * 8 :: h,coordinates(3),Momentum(3),t,t_f
  real * 8 :: X(5),X1(5),g(8)
  real * 8 :: Y0(5),Y1(5),Y2(5),Y3(5),Y4(5),Y5(5),Y6(5)
  real * 8 :: Y7(5),Y8(5),Y9(5),Y10(5),Y11(5),Y12(5)
  real * 8 :: Energy,Carter,parameters(9)
  integer :: i
  100 format(6f25.10)
  200 format(1f10.4,2f25.15)
...
Subroutine Differential_Eq(X1,Y)
  ...
  real * 8 :: g(8),Derivative_of_parameters(10)
  real * 8 :: parameters(9)
  ...
  Y(1) = R_Dot(g,parameters,Temp_X,Temp_P)                          
  Y(2) = Theta_Dot(g,parameters,Temp_X,Temp_P)                          
  Y(3) = Phi_Dot(g,parameters,Temp_X,Temp_P)                          
  Y(4) = P_R_Dot(g,parameters,Temp_X,Temp_P,Derivative_of_parameters) 
  Y(5) = P_Theta_Dot(g,parameters,Temp_X,Temp_P,Derivative_of_parameters) 
  return 
  end Subroutine
